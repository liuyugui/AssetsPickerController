...
- (void)setupCollectionView
{
    ...
    self.collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 44, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - 44 -48) collectionViewLayout:layout];
...
- (void)reloadData
{
    ...
    [self.btnDone setTitle:[NSString stringWithFormat:@"确定(%lu)",(unsigned long)self.collectionView.indexPathsForSelectedItems
                            .count] forState:UIControlStateNormal];
    [self showNoAssetsIfNeeded];
}
- (void)setAssetsCountWithSelectedIndexPaths:(NSArray *)indexPaths
{
    [self.btnDone setTitle:[NSString stringWithFormat:@"确定(%lu)",(unsigned long)indexPaths.count] forState:UIControlStateNormal];
}
